...
parser.add_argument("--enemy_type", dest="enemy_type",default="78")
...
# runs simulation
def simulation(env,x,test_mode=False):
    f,p,e,t = env.play(pcont=x)
    filename = os.path.join(experiment_name,"ind_gain.txt")
    if(test_mode==True):
...
# evaluation
def evaluate(x,test_mode=False): 
    if (test_mode == True):
        return np.array(list(map(lambda y: simulation(env,y,True), x)))
    else:
